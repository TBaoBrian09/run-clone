...
import { Flex, LinkExternal, Text, useModal, Skeleton,Button, SearchIcon } from '@phamphu19498/runtogether-uikit';
...
import {  ChevronRightIcon, Progress, Radio, StatusClosedIcon, StatusOpenIcon } from 'components/Pancake-uikit';
...
import { FetchMinToWin } from './hook/fetchMinToWin';
...
function BannerVotes() {
  return (
    <>   
      <BannerVote >         
          {totalVoting.isGreaterThanOrEqualTo(stakeMinToWin) && <CsTextStatus color="#fff">{convertdataAgree.isGreaterThan(convertdataRefuse) ? renderPercentAgree : renderPercentRefuse }<span style={{fontSize:'20px'}}>%</span></CsTextStatus>}
          {totalVoting.isGreaterThanOrEqualTo(stakeMinToWin) ?  <CsTextStatus color="#fff">{convertdataAgree.isGreaterThan(convertdataRefuse) && totalVoting.isGreaterThan(stakeMinToWin) ? 'Agree' : 'Disagree' }</CsTextStatus> :  <CsTextStatus color="#fff">Cancel</CsTextStatus>}
          <ButtonVoteStatus>
  ...
  const convertTotalVote = new BigNumber(dataVoting.totalVote)
    ...
    <PageFullWidth>
      ...
      <Container>
        <WrapAppBody>
          ...
          {isTimeCanUnStake  ?
              totalVoting.isLessThan(stakeMinToWin) ? 
                <>
                      <TextResResult>
            ...
            <WrapCurrentResult>
              <CustomWidthResult>
                ...
                <Progress variant="round" primaryStep={Number(convertVoteAgreee.dividedBy(convertVoteAgreee.plus(convertVoteRefuse)).multipliedBy(new BigNumber(100)))} scale="sm" />
                <WrapPercent>
                  <Text fontSize='14px'>
                        { partAgreee.isNaN() ?
                          "0.00"
              ...
              <CustomWidthResult>
                <Text color="#B2B3BD">{t("Disagree")}</Text>
                <Progress bgcolor="#E75243" variant="round" primaryStep={Number(convertVoteRefuse.dividedBy(convertVoteAgreee.plus(convertVoteRefuse)).multipliedBy(new BigNumber(100)))} scale="sm" />
                <WrapPercent>
                  <Text fontSize='14px'>
                        { partRefuse.isNaN() ?
                          "0.00"
