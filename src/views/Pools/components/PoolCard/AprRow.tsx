...
import { Flex, TooltipText, IconButton, useModal, CalculateIcon, Skeleton, Text } from '@phamphu19498/runtogether-uikit'
...
const AprRow: React.FC<AprRowProps> = ({ pool, stakedBalance, performanceFee = 0 }) => {
  ...
  return (
    <Flex alignItems="center" justifyContent="space-between" style={{padding:"5px 24px"}}>
      ...
      <CustomTooltipText ref={targetRef}>{isAutoVault ? `${t('APY')}:` : `${t('APR')}:`}</CustomTooltipText> */}
      <Text color="textSecondary">{t("APR")}</Text>
      { isFinished || !apr ? (
