import React, { useState, useEffect } from 'react';
import BigNumber from 'bignumber.js';
import { useTranslation } from 'contexts/Localization';
import { useWeb3React } from '@web3-react/core';
import ConnectWalletButton from 'components/ConnectWalletButton';
import { Flex, Text, Button, Input, AutoRenewIcon } from '@phamphu19498/runtogether-uikit';
import { Progress, ArrowDownIcon, AlertIcon } from "components/Pancake-uikit";
import { MinusIcon, PlusIcon } from 'components/Pancake-uikit/widgets/Menu/icons';
import styled from 'styled-components';
import PageFullWidth from 'components/Layout/PageFullWidth';
import tokens from 'config/constants/tokens';
import { useHistory } from 'react-router-dom';
import { GetBalanceMysteryBox, GetTokenIdsMystery } from 'state/inventory';
import { fetchBalanceNft, fetchTokensId } from 'state/inventory/hook/fetchDataMysteryBox';
import { fetchBalanceMysteryBox, fetchlistTokenId } from 'state/inventory/actions';
import { useDispatch } from 'react-redux';
import { AppDispatch } from 'state';
import Header from './components/Header';
import { FetchDataUser, GetAmountSold, GetPriceBox, GetStartEndTime } from './hook/fetchData';
import StartEndTimeCol from './components/startEndTime';
import { useApprove } from './hook/useApprove';
import { useBuy } from './hook/useBuy';

const MysteryBox = () => {
    const { t } = useTranslation();
    const history = useHistory();
    const { account } = useWeb3React();
    const [ amount, setAmount ] = useState(1);
    const [ refresh, setRefresh ] = useState(Date.now());
    const { handleApprove, requestedApproval, pendingTx } = useApprove();  
    const { dataBalance } = FetchDataUser(requestedApproval, refresh);  
    const { price } = GetPriceBox();
    const { inforSold } = GetAmountSold(refresh);
    const { startEndTime } = GetStartEndTime();
    const [ isEndsIn, setIsEndsIn ] = useState(false);
    const [ isOverTimeBuy, setIsOverTimeBuy ] = useState(false);
    const [ isSoldOut, setIsSoldOut ] = useState(false);
    const converPrice = new BigNumber(price.toString()).dividedBy(new BigNumber(10).pow(18)).toNumber();
    const currentTime = Date.now();
    const [ balanceMysteryBox ] = GetBalanceMysteryBox(account);
    const [ tokenIdsMystery ] = GetTokenIdsMystery(account, balanceMysteryBox);
    const dispatch = useDispatch<AppDispatch>()
    const updateBalanceMysteryBox = async () => {
        try {
            const result = await fetchBalanceNft(account)
            dispatch(fetchBalanceMysteryBox(result)) 
            const ids = await fetchTokensId(account, result.balanceMysteryBox)
            dispatch(fetchlistTokenId(ids))               
        } catch (e) {
            console.log(e)
        }
    }
    const onRefresh = async (value) => {
        setRefresh(value);
        setAmount(1); 
        updateBalanceMysteryBox();
    }    
    const { handleBuy, pendingBuy } = useBuy(amount, onRefresh);        
    const handleMinus = () => {
        if (amount > 0) {
            setAmount(amount - 1);
        }
    } 
    const handlePlus = () => {
        if (amount < Number((inforSold.maxSell - inforSold.sold))) {
            setAmount(amount + 1);
        }
    }
    const handleChangeInput = (e) =>{
        const {value} = e.target;         
        if ((/^\d+$/.test(value) || value === '' )) {
            let convertNumber = Number(value) 
            if ( convertNumber > Number((inforSold.maxSell - inforSold.sold))) {
                convertNumber  =  Number((inforSold.maxSell - inforSold.sold))      
            }
            if (convertNumber < 0) {
                convertNumber = 0
            } 
            setAmount(convertNumber); 
        }              
    }

    function clickGotoMysteryBox() {
        const activeMystery = '1';
        history.push(`/inventory/${activeMystery}`);
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }

    useEffect(() => {
        if( startEndTime.startIn*1000 < currentTime && startEndTime.startIn !== 0 ) { // check over startIn
            setIsEndsIn(true);
        }
    }, [currentTime, startEndTime.startIn]);   
    useEffect(() => {
        if( startEndTime.endIn*1000 < currentTime && startEndTime.endIn !== 0 ) {
            setIsOverTimeBuy(true);
        }
    }, [currentTime, startEndTime.endIn]);
    useEffect(() => {
        if(inforSold.sold === inforSold.maxSell && inforSold.maxSell !== 0) {
            setIsSoldOut(true);
        }
    }, [inforSold]);
    const getSumMysteryBox = () => {
        let sumType = 0;
        if(tokenIdsMystery?.length > 0) {
            sumType = tokenIdsMystery.length;            
        }
        return sumType;
    }
    return (
        <PageFullWidth>
            <Container>
                <Header/>
                <Wrapper>
                    <PageBox>
                        <Box src="/images/mysterybox/box.png" alt='Image Mystery Box' />
                    </PageBox>
                    <PageInformation>
                        <Flex flexDirection="column">
                            <Text bold color="#23262F" fontSize='40px'>{t("Mystery Box")}</Text>
                            <Flex alignItems="center" style={{gap:"10px"}}>
                                <Text 
                                    style={{ marginTop: "5px" }} 
                                    bold fontSize='20px'
                                >
                                    {converPrice.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Text>
                                <img src= "/images/coins/0xe9e7cea3dedca5984780bafc599bd69add087d56.png" alt="logo token earn" style={{height:"30px", width:"30px"}}/>
                            </Flex>                            
                        </Flex>
                        <QuantityBox>
                            <Progress variant="round" bgcolor="#4B19F5" primaryStep={(inforSold.sold*100)/inforSold.maxSell} scale="sm"/>
                            <InforSold justifyContent="space-between">
                                <Text>{t("Sold")} {inforSold.sold} {t("boxes")}</Text>
                                <Text>{inforSold.maxSell} {t("boxes")}</Text>
                            </InforSold>
                        </QuantityBox>
                        <InforTime justifyContent="space-between">
                            {
                                !isEndsIn ? 
                                    <>
                                        <Text bold>{t("Start in")}</Text>
                                        <StartEndTimeCol startEndDate={startEndTime.startIn*1000}/>                                        
                                    </>
                                    
                                : 
                                    <>
                                        <Text bold>{t("Ends in")}</Text>
                                        <StartEndTimeCol startEndDate={startEndTime.endIn*1000}/>
                                    </>                                    
                            }                            
                            
                        </InforTime>
                        <InforAmountBuy justifyContent="space-between">
                            <Text style={{marginTop: "12px"}}>{t("Amount")}</Text>
                            <WrapCount>
                                <ButtonQuanlity onClick={ handleMinus } disabled={ amount <= 1 || isOverTimeBuy }>
                                    <MinusIcon />
                                </ButtonQuanlity>
                                <CustomInput 
                                    type="text"
                                    inputMode="numeric"
                                    scale="lg"
                                    value={amount}
                                    onChange={handleChangeInput}   
                                    readOnly={isOverTimeBuy}                                
                                />
                                <ButtonQuanlity onClick={ handlePlus } disabled={ amount >= Number((inforSold.maxSell - inforSold.sold)) || isOverTimeBuy}>
                                    <PlusIcon />
                                </ButtonQuanlity>
                            </WrapCount>
                        </InforAmountBuy>
                        <Flex mt="1rem" mb="1rem">
                            { account ?
                                <>
                                    <WrapBuyApprove flexDirection="column">
                                        <CustomButtonBuy 
                                            onClick={handleApprove}
                                            endIcon={pendingTx ? <AutoRenewIcon spin color="textDisable" /> : null}
                                            disabled={ dataBalance.isApprove !== 0 || !isEndsIn || isOverTimeBuy || isSoldOut || pendingTx }
                                        >
                                            {t("Approve")}
                                        </CustomButtonBuy>
                                        <ArrowIconCus/>
                                        {
                                            converPrice > dataBalance.balanceOf ? 
                                            <CustomButtonBuy disabled>
                                                {t("Insufficient %symbol% balance", { symbol:tokens.busd.symbol})}
                                            </CustomButtonBuy>
                                            :
                                            <>
                                                {
                                                    (amount >= 1) ? 
                                                    <CustomButtonBuy 
                                                        onClick={handleBuy}
                                                        endIcon={pendingBuy ? <AutoRenewIcon spin color="textDisable" /> : null}
                                                        disabled={ dataBalance.isApprove === 0 || !isEndsIn || isOverTimeBuy || isSoldOut || pendingBuy}
                                                    >
                                                        {t("Buy")}
                                                    </CustomButtonBuy>
                                                    :
                                                    <CustomButtonBuy disabled>
                                                        {t("Amount must be greater than 0")}
                                                    </CustomButtonBuy>
                                                }
                                            </>                                            
                                        }
                                       
                                    </WrapBuyApprove>
                                </>
                            :
                                <CustonButtonConnect/>
                            }
                        </Flex>
                        <CsInfoUserBuyBox>
                            <Text>
                                {t("Your BUSD balance:")}
                            </Text>
                            <Flex alignItems="center" style={{gap:"10px"}}>
                                <Text>
                                    {dataBalance.balanceOf.toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Text>
                            </Flex>                            
                        </CsInfoUserBuyBox>
                        <CsInfoUserBuyBox>
                            <Text>
                                {t("Your mystery boxes:")}
                            </Text>
                            <Flex alignItems="center" style={{gap:"10px"}}>
                                <Text>
                                    {getSumMysteryBox().toLocaleString('en', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </Text>
                            </Flex>                            
                        </CsInfoUserBuyBox>
                        <CsButton onClick={clickGotoMysteryBox}>
                            {t("Back to Inventory")}
                        </CsButton>  
                        {
                            isOverTimeBuy && 
                            <AlertOverTimeBuy>
                                <AlertIcon style={{marginRight: "5px"}}/>
                                <Text>{t("Time to buy a mystory box is over")}</Text>                            
                            </AlertOverTimeBuy>
                        }   
                        {
                            isSoldOut && 
                            <AlertOverTimeBuy>
                                <AlertIcon style={{marginRight: "5px"}}/>
                                <Text>{t("Sold out")}</Text>                            
                            </AlertOverTimeBuy>                        
                        }     
                    </PageInformation>
                </Wrapper>
            </Container>
        </PageFullWidth>
    );
}

export default MysteryBox

const Container = styled.div`
    width: 100%;
    height: auto;
    display: flex;
    max-width: 1320px;
    flex-direction:column;
    margin-top: 76px;
    @media screen and (max-width: 600px) {
        flex-direction: column;
        width: 100%;
    }
        @media screen and (min-width: 601px) and (max-width: 768px) {
        flex-direction: column;
    }
`
const Wrapper = styled(Flex)`
    width: 100%;
    flex-direction: row;
    @media screen and (max-width: 768px) {
        flex-direction:column-reverse;
    }
`
const PageBox = styled.div`
    width: 50%;
    height: auto;
    padding: 3rem;
    @media screen and (max-width: 600px) {
        width: 100%;
        margin-left: 0;
        padding: 15px;
    }
    @media screen and (min-width: 601px) and (max-width: 768px) {
        width: 100%;
        margin-left: 0;
        margin-top:0rem;
    }
    @media screen and (min-width: 769px) and (max-width: 1080px) {
        width: 50%;
        margin-left: 0;
        padding-top: 1rem;
        margin-top:0rem;
    }
`
const Box = styled.img`
    width: 100%;
    height: auto;
    @media screen and (max-width: 600px) {   
    }
        @media screen and (min-width: 601px) and (max-width: 768px) {

    }
`
const PageInformation = styled.div`
    padding: 3rem;
    width: 50%;
    display: flex;
    flex-direction: column;
    @media screen and (max-width: 600px) {
        width: 100%;
        margin-left: 0;
        padding: 15px;
    }
    @media screen and (min-width: 601px) and (max-width: 768px) {
        width: 100%;
        margin-left: 0;
        margin-top:1rem;
    }
    @media screen and (min-width: 769px) and (max-width: 1080px) {
        width: 50%;
        margin-left: 0;
        padding-top: 0rem;
        margin-top:0rem;
    }
`
const QuantityBox = styled.div`
    width: 100%;
    margin-top: 30px;
`
const InforSold = styled(Flex)`
    margin-top: 10px;
`
const InforTime = styled(Flex)`
    margin-top: 25px;
`
const InforAmountBuy = styled(Flex)`
    margin-top: 25px;
`
const WrapCount = styled(Flex)`
    align-items:center;
    gap:10px;
`
const WrapBuyApprove = styled(Flex)`
    width: 100%;
    margin-top: 20px;
`
const ButtonQuanlity = styled(Button)`
  background: transparent;
  box-shadow: none;
  border: 2px solid #E6E8EC;
  border-radius: 100px;
  width: 42px;
  height: 42px;
  padding: 4px;
`
const CustomInput = styled(Input)`
    background: none;
    color: black;
    width: 100%;
    padding: 0;
    width: 50px;
    text-align: center;
    border: none;
    box-shadow: none;
`
const CustonButtonConnect = styled(ConnectWalletButton)`
    text-transform:uppercase;
    font-weight:bold;
    border-radius:90px;
    width:100%;
    height: 54px;
    background: #FF592C;
    box-shadow:none;
    color:#fff;
`
const CustomButtonBuy = styled(Button)`
  text-transform:uppercase;
  font-weight:bold;
  border-radius:90px;
  width:100%;
  box-shadow:none;
`
const ArrowIconCus = styled(ArrowDownIcon)`
   width: 30px;
   margin-top: 20px;
   margin-bottom: 20px;
`
const AlertOverTimeBuy = styled.div`
    border: 2px solid #FF592C;
    padding-top: 15px;
    padding-bottom: 15px;
    border-radius: 10px;
    margin-top: 25px;
    display: flex;
    flex-direction: row;
    justify-content: center;
`
const CsInfoUserBuyBox = styled(Flex)`
    margin-top: 10px;
    justify-content: space-between;
    align-items: center;
`

const CsButton = styled(Button)`
    height: 48px;
    margin-top:1rem;
    border: 2px solid #E6E8EC;
    box-shadow:none;
    border-radius:90px;
    background:transparent;
    color: #000;
    &:hover{
        background:#E75243;
        color: #FFF;
        opacity: 1;
    }
`