...
import { fetchItemsSellByUser } from 'state/marketplace/hook/fectDataMarketPlace'
...
const Delist: React.FC<Props> = ({ nftId, saleId, price, boxType, onDismiss, onRefresh }) => {
  ...
  const { handleDelist, requestedDelist, pendingDelist, isClose } = useDelist(saleId, onRefresh)
  ...
  useEffect(() => {
    if(requestedDelist) {
      const getTotalSellItems = async () => {
        try {
          const result = await fetchItemsSellByUser(totalSell, account)
  ...
  }, [requestedDelist]) // eslint-disable-line react-hooks/exhaustive-deps
